...
# save to csv file 
np.savetxt('../../solutions/test_Shake/pt3d_list.csv', pt3d_list, delimiter=',', fmt='%.8f')

for i in range(ncam):
    np.savetxt('../../inputs/test_Shake/pt2d_list_cam' + str(i+1) + '.csv', pt2d_list_all[i], delimiter=',', fmt='%.8f')
...
np.savetxt('../../solutions/test_Shake/pt3d_list_img.csv', pt3d_list, delimiter=',', fmt='%.8f')
...
for i in range(ncam):
    ...
    # save to tiff files
    cv2.imwrite('../../inputs/test_Shake/img_cam'+str(i+1)+'.tif', img_list[i])

# %%
# load results 
tr3d_list = np.loadtxt('../../results/test_Shake/tr3d_img_deleteGhost.csv', delimiter=',', skiprows=1)
